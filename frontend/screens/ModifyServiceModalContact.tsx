...
export default function ModifyServiceModalContact({
  navigation,
  route,
}: ServiceStackScreenProps<'ModifyServiceModalContact'>) {
  const token = useAppSelector((state) => state.userCredential.userToken);
  const toast = useToast();
  const { serviceName, serviceDescription, servicePrice, service } =
    route.params;
  interface service {
  ...
  //     const serviceCreation = await modifyService(
  //       serviceName,
  //       serviceDescription,
  //       servicePrice,
  //       contactNumber,
